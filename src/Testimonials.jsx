  ...
  return (
    <div id="testimonials" className="px-6 pt-10 mt-12 bg-white text-black">
      ...
      <div className="bg-[#161820] rounded-3xl px-6 py-14 relative">
        <div
          ref={containerRef}
          className="flex space-x-6 overflow-x-auto no-scrollbar min-w-full snap-x snap-mandatory"
        >
          {testimonials.map((testimonial, index) => (
            <div
              key={testimonial.id}
              className="flex-shrink-0 w-full max-w-xs sm:max-w-sm md:max-w-lg relative text-white snap-center"
            >
              <div className="border border-[#9ef04a] rounded-3xl p-6 sm:p-8">
        ...
        <div className="flex justify-center items-center space-x-10 mt-16 text-white text-xl select-none">
          ...
          <div className="flex space-x-2 text-[#9ef04a] cursor-pointer">
            {testimonials.map((_, index) => (
              <i
                key={index}
                className={`fa-star ${
                  index === activeIndex ? 'fas text-[#9ef04a]' : 'far text-white'
                }`}
